...
// Twitter stuff
	...
	{
		listID = uri.replace(/\//g, "-");
		if ($('#group-list' + listID + ' li').length > 0)
		{
			toggle_group(listID);
		}
		else
		{
			$('#group-list' + listID).html("<div align='center'><br /><br /><br /><img src='/image/ajaxsm.gif'></div>");
			proxy({
				...
				success: function(response){
					...
					console.log('#group-list' + listID);
					$('#group-list' + listID).html(html);
					toggle_group(listID);
				}
			});		
		}
	...
	function display_lists(lists)
	{
		...
		if (lists.length > 0)
		{
			...
			{
				var list = lists[i];
				var friendlyName = list.uri.replace(/\//g, "-")
				html  =  '<a href="#list=' + list.uri + '"><div>' + list.uri.replace("/", "@") + '</div></a>';
				html  += '<ul id="group-list' + friendlyName + '" class="group-list"></ul>';
