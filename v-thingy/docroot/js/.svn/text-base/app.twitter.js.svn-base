...
function App_Twitter_API(application_data)
{
			...
			<div class='tweet-"+item.unread+" tweet' style='clear:left; display:none' id='tweetid-"+item.id+"'> \
				...
				<div class='tweet-body'> \
					...
					<div class='tweet-footer'> \
						" + item.from_name + " said <a href='" + external_link + "' target='_blank' title='" + (item.created_at) + "' class='timestamp'>"+relative_time(item.created_at)+"</a> on "+ this.settings.title + " \
	...
	{
		$("#maincontainer").show();
		$('#tweet-list').prepend(this.tweet_to_html(tweet));
	...
	this.post_new_tweet = function(tweet)
	{
		...
		$.post(
			...
			function(call){
				...
				html = "<div class='small tweet'><a href='"+external_url+"' target='_blank'>#"+call.response.id+"</a> posted to <b>"+Tweenky.applications[call.params.ak].settings.title+"</b>.  <span style='font-style: italic;'>\""+call.response.text+"\"</span></div>";
