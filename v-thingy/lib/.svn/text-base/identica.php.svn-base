...
class Identica {
	...
	function Identica($username, $password) {
	...
	function getFriendsTimeline($format, $since_id = NULL, $page = 1, $count = 200) {
		$api_call = sprintf("/statuses/friends_timeline.%s?page=%s&count=%s", $format, $page, $count);
		if ($since_id != NULL) {
	...
	function updateStatus($status, $reply_to_status_id = null) {
		...
		$api_call = sprintf("/statuses/update.xml?status=%s", $status);
	...
	function getFriends($format, $id = NULL, $page = 1) {
		if ($id != NULL) {
			$api_call = sprintf("/statuses/friends/%s.%s?page=%s", $id, $format, $page);
			$authenticate = false;
		}
		else {
			$api_call = sprintf("/statuses/friends.%s?page=%s", $format, $page);
			$authenticate = true;
		}
	...
	function getFollowers($format, $lite = NULL) {
		$api_call = sprintf("/statuses/followers.%s%s", $format, ($lite) ? "?lite=true" : NULL);
		return $this->APICall($api_call, true);
	}
	...
	*/
	function friendshipExists($format, $user_a, $user_b) {
		$api_call = sprintf("/friendships/exists.%s?user_a=%s&user_b=%s", $format, $user_a, $user_b);
		return $this->APICall($api_call, true);
	}
	/*
	...
	*/
	function createFriendship($format, $id) {
		$api_call = sprintf("/friendships/create/%s.%s", $id, $format);
		return $this->APICall($api_call, true, true);
	}
	...
	function verifyCredentials() {
		$api_call = sprintf("/account/verify_credentials.xml");
	...
	function getFavorites($format, $id = NULL, $page = 1) {
		if ($id == NULL) {
			$api_call = sprintf("/favorites.%s", $format);
		}
		else {
			$api_call = sprintf("/favorites/%s.%s", $id, $format);
		}
		if ($page > 1) {
			$api_call .= sprintf("?page=%d", $page);
		}
	...
	private function APICall($api_url, $require_credentials = false, $http_post = false) {
		...
		curl_setopt($curl_handle, CURLOPT_URL, "http://identi.ca/api".$api_url);
